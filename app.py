...
st.markdown("<p class='tip-text'>페트리 디시 사진을 업로드하면 콜로니를 자동으로 감지하고 개수를 세어드립니다.<br>밝은 배지 위의 흰색/불투명 콜로니에 최적화되어 있습니다.</p>", unsafe_allow_html=True)
st.markdown("---")
...
with st.expander("⚙️ 감지 파라미터 조정", expanded=False):
    ...
    with col1:
        tophat_thresh = st.slider("감지 민감도 (Top-hat 임계값)", 5, 40, 18, help="낮을수록 더 많이 감지 (노이즈 증가 가능). 기본값 18")
        min_area = st.slider("최소 콜로니 면적 (px²)", 10, 300, 40, help="이보다 작은 점은 무시합니다")
    with col2:
        max_area = st.slider("최대 콜로니 면적 (px²)", 1000, 30000, 12000, help="이보다 큰 영역은 무시합니다")
        min_circ = st.slider("최소 원형도", 0.1, 0.8, 0.3, step=0.05, help="1.0이 완전한 원. 낮을수록 불규칙한 모양도 감지")
        dish_margin = st.slider("디시 테두리 제외 비율", 0.80, 0.98, 0.90, step=0.01, help="디시 반지름의 몇 %까지 분석할지")
...
def detect_colonies(image_array, tophat_thresh, min_area, max_area, min_circ, dish_margin):
    ...
    # 2. Top-hat 변환 (밝은 배지 위 밝은 콜로니 추출)
    ...
    for cnt in contours:
        ...
        cv2.circle(result, (int(x), int(y)), int(r) + 2, (57, 211, 83), 3)
...
if uploaded:
    ...
    with st.spinner("콜로니 감지 중..."):
        result_array, colony_count = detect_colonies(img_array, tophat_thresh, min_area, max_area, min_circ, dish_margin)
    ...
    • 노이즈가 많이 잡힐 때 → 감지 민감도 높이기, 원형도 높이기
    </p></div>
    """, unsafe_allow_html=True)
