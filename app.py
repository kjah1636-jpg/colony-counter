...
with st.expander("⚙️ 감지 파라미터 조정", expanded=True):
    ...
    with col1:
        ...
        adapt_block = st.slider("감지 블록 크기", 21, 101, 51, step=10,
            help="콜로니보다 커야 합니다. 콜로니가 크면 높이세요.")
        adapt_C = st.slider("배경 대비 민감도", -20, -3, -8, step=1,
            help="음수가 클수록(예: -15) 더 많이 감지. 노이즈 증가 가능.")
        clahe_clip = st.slider("대비 향상 강도", 1.0, 6.0, 4.0, step=0.5,
            help="높일수록 흐릿한 콜로니가 더 잘 보입니다.")
    with col2:
        dist_thresh = st.slider("콜로니 분리 민감도", 0.1, 0.5, 0.2, step=0.05,
            help="낮을수록 붙어있는 콜로니를 더 잘 분리합니다.")
        min_area = st.slider("최소 콜로니 면적 (px²)", 5, 200, 25,
            help="이보다 작은 점은 무시합니다.")
        max_area = st.slider("최대 콜로니 면적 (px²)", 200, 8000, 2000,
            help="이보다 큰 영역은 무시합니다.")
        min_circ = st.slider("최소 원형도", 0.1, 0.7, 0.3, step=0.05,
...
def get_dish_mask(gray, margin):
    ...
    for param2 in [40, 30, 20, 15, 10]:
        circles = cv2.HoughCircles(blurred, cv2.HOUGH_GRADIENT, dp=1.2, minDist=100,
            ...
            minRadius=int(min(h, w) * 0.30), maxRadius=int(min(h, w) * 0.56))
...
def detect_colonies(image_array, clahe_clip, adapt_block, adapt_C,
    ...
    # 4) 적응형 임계값 (핵심 알고리즘)
    #    blockSize: 주변 블록 크기 (홀수여야 함)
    #    C: 평균보다 C만큼 더 밝아야 콜로니로 판단 (음수 = 더 관대)
    ...
    if dist.max() == 0:
        return img, 0, binary, binary, dish_info
...
if uploaded:
    ...
    with st.spinner("🔬 콜로니 감지 중..."):
        result_array, colony_count, debug_binary, debug_fg, dish_info = detect_colonies(
            img_array, clahe_clip, adapt_block, adapt_C,
            dist_thresh, min_area, max_area, min_circ, dish_margin
        )
    ...
    with st.expander("🔍 디버그: 중간 처리 결과", expanded=False):
        ...
        with dc2:
            ...
            st.image(debug_fg, use_container_width=True,
                     caption="여기서 면적/원형도 필터 후 최종 카운트됩니다.")

    st.markdown("---")
    st.markdown(f"<div class='big-count'>{colony_count}</div>", unsafe_allow_html=True)
    st.markdown("<div class='count-label'>colonies detected · Adaptive Threshold + Distance Transform</div>",
                unsafe_allow_html=True)
    ...
    st.markdown("""
    <div class='card'><p class='tip-text'>
    📌 <strong>시작 전 파라미터 안내</strong><br><br>
    • <strong>배경 대비 민감도</strong>: 기본값 -8. 콜로니가 안 잡히면 -12까지 낮춰보세요.<br>
    • <strong>감지 블록 크기</strong>: 콜로니 지름의 2~3배 권장. 기본값 51.<br>
    • <strong>최소 콜로니 면적</strong>: 작은 콜로니를 잡으려면 낮추고, 노이즈가 많으면 높이세요.<br>
    • <strong>디버그 탭</strong>: 이진화 결과를 보면 어느 파라미터를 조정할지 바로 알 수 있습니다.
    </p></div>
    """, unsafe_allow_html=True)
