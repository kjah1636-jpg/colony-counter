...
st.markdown(
    "<p class='tip-text'>페트리 디시 사진을 업로드하면 콜로니를 자동으로 감지하고 개수를 세어드립니다.<br>"
    "밝은 배지 위의 흰색/불투명 콜로니 + 플라스틱 반사 제거에 최적화되어 있습니다.</p>",
    unsafe_allow_html=True
)
st.markdown("---")
...
with st.expander("⚙️ 감지 파라미터 조정", expanded=False):
    ...
    with col1:
        tophat_thresh = st.slider("감지 민감도 (Top-hat 임계값)", 5, 40, 18,
            help="낮을수록 더 많이 감지합니다. 노이즈가 늘면 높이세요.")
        min_area = st.slider("최소 콜로니 면적 (px²)", 10, 500, 250,
    ...
    with col2:
        ...
        min_circ = st.slider("최소 원형도", 0.1, 0.8, 0.35, step=0.05,
            help="1.0이 완전한 원. 낮을수록 불규칙한 모양도 감지합니다.")
...
def detect_colonies(image_array, tophat_thresh, min_area, max_area, min_circ, dish_margin):
    ...
    circles = cv2.HoughCircles(
        blurred_hough, cv2.HOUGH_GRADIENT, dp=1.2, minDist=200,
        param1=50, param2=30,
