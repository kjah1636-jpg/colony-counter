...
st.markdown("# 🔬 Colony Counter Pro v4")
st.markdown("---")
...
def split_blob_watershed(comp_mask, img_rgb, dist_thresh):
    try:
        ...
        markers = np.int32(markers) + 1  # watershed는 반드시 int32
        markers[unknown == 255] = 0

        h, w = comp_mask.shape[:2]
        img_crop = np.uint8(img_rgb[:h, :w])
        if img_crop.ndim == 2:
...
def detect_colonies(image_array, params):
    ...
    # ── 5. 채널 합성 ─────────────────────────────────────
    working = cv2.addWeighted(tophat_ch, 0.75, base_ch, 0.25, 0)
    if use_color:
    ...
    binary_clean = cv2.morphologyEx(binary_clean, cv2.MORPH_CLOSE, k3, iterations=1)
    ...
    split_threshold = max_area * 1.0   # 이보다 크면 복수 콜로니로 간주
...
with pcol1:
    with st.expander("⚙️ 기본 파라미터", expanded=True):
        ...
        adapt_C     = st.slider("감지 민감도 (C)", -20, -1, -10, 1,
            help="낮을수록 더 민감 (노이즈 증가 가능). 기본 -10")
        clahe_clip  = st.slider("대비 향상 강도", 1.0, 8.0, 4.0, 0.5)
        dist_thresh = st.slider("콜로니 분리 민감도", 0.10, 0.50, 0.15, 0.05,
...
with pcol2:
    with st.expander("⚙️ 고급 파라미터", expanded=True):
        ...
        min_circ    = st.slider("최소 원형도", 0.10, 0.70, 0.10, 0.05,
            help="낮을수록 불규칙 모양도 감지")

        st.markdown("""<p style='color:#8b949e;font-size:13px'>
        <b>최소 크기</b>는 1500px 리사이즈 기준 픽셀 면적 (px²)입니다.<br>
        작은 점까지 잡으려면 낮추세요 (기본 8).
        </p>""", unsafe_allow_html=True)
        min_area_px  = st.slider("최소 콜로니 면적 (px²)", 5, 200, 8, 1,
            help="1500px 리사이즈 기준. 항상 일정한 스케일.")
        max_area_pct = st.slider("최대 콜로니 크기 (접시면적 %)", 0.5, 20.0, 5.0, 0.5,
            format="%.1f",
...
if uploaded:
    ...
    st.markdown("""
    <div class='card'><p class='tip-text'>
    <strong>v4 핵심 개선</strong> (v3 대비)<br><br>
    ✅ <strong>소형 콜로니 감지 강화</strong><br>
    &nbsp;&nbsp;최소 면적 기본값 20 → 8 px² (작은 점까지 감지)<br>
    &nbsp;&nbsp;멀티스케일 이진화 블록 크기 추가 (11px 포함)<br><br>
    ✅ <strong>감도 기본값 최적화</strong>: C=-10, CLAHE=4.0<br>
    ✅ <strong>원형도 필터 완화</strong>: 0.20 → 0.10 (불규칙 모양 감지)<br>
    ✅ <strong>클러스터 분리 강화</strong>: 분리 임계값 낮춤<br><br>
