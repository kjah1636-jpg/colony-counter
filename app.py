...
def load_cellpose_model():
    ...
    # cellpose >= 3.x uses CellposeModel; fallback to Cellpose for older versions
    if hasattr(models, "CellposeModel"):
        return models.CellposeModel(model_type="cyto3", gpu=False)
    else:
...
def detect_cellpose(image_array, cell_diameter, min_area, max_area, dish_margin):
    ...
    # Handle both 3-return and 4-return signatures
    masks = result[0]
    flows, styles = result[1], result[2]
